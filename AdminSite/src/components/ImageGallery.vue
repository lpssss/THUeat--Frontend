...
<template>
  ...
  />
  <ImagesUploader ref="imageUploader"/>
  <q-btn
    color="primary"
    label="保存修改"
    @click="saveChanges"
...
export default {
  ...
  setup(props) {
    ...
    const myImages=reactive([...props.images])
    const imagesCount=computed(()=>myImages.length)
    ...
    function confirmDeleteImage (link) {
      $q.dialog({
        ...
        const idx = myImages.indexOf(link);
        console.log(idx)
        myImages.splice(idx, 1);
      })
    }
    //功能：点击”保存修改“按钮后，将需上传或删除的图片储存到后端，根据后端回复显示操作结果
    function saveChanges(){
      imageUploader.value.uploadImages()
    }
