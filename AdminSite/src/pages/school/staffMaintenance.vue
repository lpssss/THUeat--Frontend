<template>
  <div class="q-pa-md" v-if="editable == false">
    ...
    <template v-slot:top-right>
      ...
      <template v-slot:body-cell-status="props">
        <q-td :props="props">
          <q-toggle v-model="props.row.staffStatus" 
                    checked-icon="check" 
                    color="green" 
  ...
  <div class="q-pa-md" style="margin-left:10%; margin-right:10%;" v-if="editable == true">
    ...
    <div class="q-gutter-md" style="max-width: 100%">
      <q-input filled v-model="newStaff.staffVaildName" label="档主姓名" stack-label />
      <q-input filled v-model="newStaff.staffPhone" label="档主联络号码" stack-label />
      <q-select filled v-model="newStaff.canteenName" :options="canteen" label="食堂" />
      <q-select filled v-model="newStaff.stallFloor" :options="floor" label="楼层" />
      <q-select filled v-model="newStaff.stallName" :options="stall" label="档口" />
...
export default defineComponent({
  setup () {
    ...
    // api.post('Api_Link', {
    //   staffStatus: staffs.staffStatus.value,
    // }).then ((res) => {
      // if (res.status === 200 && res.data !== undefined) {
      //     updateToken(res.data.token);
      // }
